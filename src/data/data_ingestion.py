...
class DataIngestion:
    ...
    def load_datasets(self):
        try:
            ...
            self.sensor_data = pd.read_csv(TELEMETRY_DATA_PATH, low_memory=False)

            logging.info("Loading maintenance dataset...")
            self.maintenance_data = pd.read_csv(MAINTENANCE_DATA_PATH)

            logging.info("Loading failure dataset...")
            self.failure_data = pd.read_csv(FAILURE_LABELS_PATH)

            logging.info("Loading equipment dataset...")
            self.equipment_data = pd.read_csv(EQUIPMENT_MASTER_PATH)
